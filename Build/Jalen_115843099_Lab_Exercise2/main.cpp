...
void list::addtitle(std::string t, std::string a, int y)
{
    ...
    if (head == NULL) {
        ...
        if(new_node->year < head->year){
            ...
            node *p1 = head;
            node *p2 = p1->next;
            while(p2 !=NULL && new_node->year >= p2->year){
                p1 = p2;
                p2 = p2->next;
            }
            p1->next = new_node;
...
int main(){
    ...
    list.addtitle("song1", "author1", 1904);
    list.addtitle("song4", "author4", 1901);
    list.addtitle("song2", "author2", 1903);
    list.addtitle("song3", "author3", 1902);

    list.removetitle("song2");
